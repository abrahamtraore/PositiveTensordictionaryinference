...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  7 08:48:52 2017
...
fontsize=16
BASE_PATH="/Users/Traoreabraham/Desktop/FinalImagesExperiences/ToyPerformances.h5"
...
Testsamples=np.array([288,256,200,200,136])

SdTucker,SdNMFVec,SdNMFMean,SdNMFMax=StandarddeviationToy(Testsamples,PerfTucker,PerfNMFVec,PerfNMFMean,PerfNMFMax)
...
SdTucker,SdNMFVec,SdNMFMean,SdNMFMax=StandarddeviationToy(Testsamples,PerfTucker,PerfNMFVec,PerfNMFMean,PerfNMFMax)
Trainingsamples=np.array([24,64,104,152,202],dtype=int)
fig, ax = plt.subplots()
ax.errorbar(Trainingsamples, PerfTucker, yerr=SdTucker, color='black', ls='--', marker='o', capsize=5, capthick=1, ecolor='red')
ax.errorbar(Trainingsamples, PerfNMFMean, yerr=SdTucker, color='black', ls='--', marker='o', capsize=5, capthick=1, ecolor='red')
plt.show()
pdb.set_trace()
...
#axe_list.errorbar(Training_samples,PerfTucker,SdTucker, color='red', ls='--', marker='o', capsize=5, capthick=1, ecolor='black')
#plt.show()
#pdb.set_trace()
...
SdTucker,SdNMFMean,SdNMFMax=StandarddeviationDcase(PerfTucker,PerfNMFMean,PerfNMFMax)
#print(SdTucker)
#print(SdNMFMean)
#print(SdNMFMax)
data=np.zeros((10,3))
...
for i, (y_arr,label) in enumerate(zip(data.T,label)):
    print(i)
    print(y_arr)
    ax = plt.plot(num_atoms, y_arr, style[i], label = label, linewidth = 1, markersize = 8)
