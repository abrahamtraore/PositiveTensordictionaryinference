...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 15 12:00:25 2017
...
def _update_projected_gradient_w(X, W, H, tolW, nls_max_iter, alpha, l1_ratio,sparseness,penalty_nature,beta, eta):
    """Helper function for _fit_projected_gradient"""
    n_samples, n_features = X.shape
    n_components_ = H.shape[0]
    #Portion of comparison in _nnls_subproblem
    #For l1_ratio = 0 the penalty is an L2 penalty.
    #For l1_ratio = 1 it is an L1 penalty.
    #For 0 < l1_ratio < 1, the penalty is a combination of L1 and L2.
    Alpha=alpha
    L1_ratio=l1_ratio
    if(penalty_nature=="L1"):
       L1_ratio=1
       Alpha=alpha*l1_ratio
    if(penalty_nature=="L2"):
       Alpha=0
    #print("The value of Alpha for W update is") 
    #print(Alpha)    
    if sparseness is None:
        Wt, gradW, iterW = _nls_subproblem(X.T, H.T, W.T, tolW, nls_max_iter,alpha=Alpha, l1_ratio=L1_ratio)
...
def _update_projected_gradient_h(X, W, H, tolH, nls_max_iter, alpha,l1_ratio,sparseness,penalty_nature,beta, eta):
    #Portion of comparison in _nnls_subproblem
    #For l1_ratio = 0 the penalty is an L2 penalty.
    #For l1_ratio = 1 it is an L1 penalty.
    #For 0 < l1_ratio < 1, the penalty is a combination of L1 and L2.
    ...
    #print("The value of lratio for H update is") 
    #print(Alpha)
    if sparseness is None:
...
def _fit_coordinate_descent(X, W, H, tol=1e-4, max_iter=2000, alpha=0.001,
    ...
    if (regularization=='mixed_transformation'):
        l2_W=(1. - l1_ratio) * alpha
        l1_H = l1_ratio * alpha
