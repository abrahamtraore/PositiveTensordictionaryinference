...
for i in range(5):
    ...
    for train_index, valid_index in skf1.split(matrix_trainvalid,y_trainvalid):
        ...
        matrix = MethodsTSPenTestAB.Cross_val_AR(Tensor_train, y_train, Tensor_valid,y_valid,C_values,G_values,Lambda_info,Alpha,Lratio,T,nbclasses,kf,kt,maximum_iterations,max_iter_update,tol,InitStrat,parallel_decision,pool)        
        sum_matrix += matrix
    ...
    G,A_f,A_t,B_f,B_t,error_list,nb_iter=MethodsTSPen.PenalizedTuckerTSPen(Tensor_train,y_train,nbclasses,kf,kt,maximum_iterations,max_iter_update,tol,InitStrat,lambda_info,lambdaG,lambda_Af,lambda_At,lambda_Bf,lambda_Bt,T,parallel_decision,pool)

    penalty=np.power(10,-7,dtype=float)
    ...
    with h5py.File('Perf_toy_tucker' + str(nb_train)+ '.h5','w') as hf:
