...
def Test_features_extraction(Testing_tensor,A_f,A_t):
    ...
    result=np.zeros((number_testing_features,sizef[1]))
    ...
    for k in range(number_testing_features):        
        ...
        feature=np.reshape(feature,(sizef[1],sizet[1]))
        result[k,:]=np.mean(feature,axis=1)
    return result
