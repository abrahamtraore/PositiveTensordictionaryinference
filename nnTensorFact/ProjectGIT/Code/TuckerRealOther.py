...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 30 22:57:18 2017
...
for i in range(5):
  ...
  TensorTFR=Tensor+eta*np.random.rand(400,width,length)        #eta*np.maximum(np.random.normal(loc=0,scale=1,size=(400,width,length)),0)      #eta*np.random.rand(400,width,length)           #eta*np.maximum(np.random.normal(loc=0,scale=1,size=(400,width,length)),0)           #eta*np.random.rand(400,width,length)
